...
along with a Linux kernel module for monitoring and enforcing
memory usage.

The system demonstrates container concepts such as process
isolation, supervision, IPC, logging, and resource control.
...
Supervisor Process
        |
        +--> Container (clone + chroot)
        |
        +--> ioctl --> Kernel Module

Kernel Module:
- Reads process RSS (memory usage)
- Generates soft limit warnings
- Enforces hard limit using SIGKILL
...
1. Multi-container supervision
   Multiple containers run under a single supervisor.

2. Metadata tracking
   Container ID, PID, and state displayed using:
   ./engine ps

3. Logging
   Container output stored in:
   ../rootfs-alpha/logs/<container>.log

4. CLI and IPC
   Commands sent from CLI to supervisor using UNIX sockets.
...
6. CPU scheduling experiment
   Comparison between normal process and container execution.

7. Clean teardown
   Containers stopped properly and no zombie processes remain.
...
including creation, monitoring, enforcement, and cleanup using
both user-space and kernel-space components.
